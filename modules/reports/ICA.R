  ...
  calcICA <- observe({
    ...
    isolate({
      if(is.numeric(database[['data']]$Kennedy)) {
        ...
        #Parse for plotly
        days_over_list = list()
        days_over_df_verde <- vector(mode="numeric", length=0)
        days_over_df_amarillo <- vector(mode="numeric", length=0)
        days_over_df_naranja <- vector(mode="numeric", length=0)
        days_over_df_rojo <- vector(mode="numeric", length=0)
        days_over_df_purpura <- vector(mode="numeric", length=0)
        days_over_df_marron <- vector(mode="numeric", length=0)
        ...
        for (i in 2:length(database[['data']])) {
          ...
          if (database$currentData == 'pm2.5') {
            ...
            days_over_df_verde[[i-1]] <- sum(dataAvged$var >= 0 & dataAvged$var <= 12, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_amarillo[[i-1]] <- sum(dataAvged$var > 12 & dataAvged$var <= 36, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_naranja[[i-1]] <- sum(dataAvged$var > 36 & dataAvged$var <= 56, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_rojo[[i-1]] <- sum(dataAvged$var > 56 & dataAvged$var <= 150, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_purpura[[i-1]] <- sum(dataAvged$var > 150 & dataAvged$var <= 250, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_marron[[i-1]] <- sum(dataAvged$var > 250, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
          }  
          if (database$currentData == 'pm10') {
            days_over_df_verde[[i-1]] <- sum(dataAvged$var >= 0 & dataAvged$var <= 54, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_amarillo[[i-1]] <- sum(dataAvged$var > 54 & dataAvged$var <= 154, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_naranja[[i-1]] <- sum(dataAvged$var > 154 & dataAvged$var <= 254, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_rojo[[i-1]] <- sum(dataAvged$var > 254 & dataAvged$var <= 354, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_purpura[[i-1]] <- sum(dataAvged$var > 424 & dataAvged$var <= 504, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
            days_over_df_marron[[i-1]] <- sum(dataAvged$var > 504, na.rm = TRUE)/sum(!is.nan(dataAvged$var))
          }
        ...
        output$daysOver <- renderPlotly({
          p = plot_ly(
            ...
            y = c(days_over_df_verde),
            name = "Dias en verde",
            type = "bar"
          )
          amarillo <- add_trace(p , x = names(database[['data']])[-c(1)], y = c(days_over_df_amarillo), name = "Dias en amarillo", type = "bar")
          naranja <- add_trace(amarillo , x = names(database[['data']])[-c(1)], y = c(days_over_df_naranja), name = "Dias en naranja", type = "bar")
          rojo <- add_trace(naranja , x = names(database[['data']])[-c(1)], y = c(days_over_df_rojo), name = "Dias en rojo", type = "bar")
          purpura <- add_trace(rojo , x = names(database[['data']])[-c(1)], y = c(days_over_df_purpura), name = "Dias en purpura", type = "bar")
          marron <- add_trace(purpura , x = names(database[['data']])[-c(1)], y = c(days_over_df_marron), name = "Dias en marron", type = "bar")
