...
descriptionUI <- function(id){
  ...
  navlistPanel(widths = c(3,9),
               tabPanel("Sobre esta herramienta",
                        fluidRow(
                          bs_carousel(id = ns("description"),use_controls = T, use_indicators = T) %>%
                            bs_set_data(interval = 10000)%>%
                            bs_append(
                              content ={div(style = "padding-left: 150px; padding-right: 150px;",wellPanel(
                                h4("Descripción:"),
